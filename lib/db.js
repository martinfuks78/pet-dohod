...
export async function initDatabase() {
  try {
    ...
    // ===== DATABASE INDEXES PRO PERFORMANCE =====
    ...
    await sql`
      ...
      ON registrations(workshop_date, workshop_location)
    `
    ...
    // Workshop indexy
    await sql`
      CREATE INDEX IF NOT EXISTS idx_workshops_start_date
      ON workshops(start_date)
    `
 ...
 */
export async function createRegistration(data, workshopVariableSymbol = null, initialStatus = 'pending') {
  ...
  const result1 = await sql`
    INSERT INTO registrations (
      ...
      ${data.notes || null}, ${initialStatus}, ${coupleGroupId}
    )
    RETURNING *
  `
...
export async function updateWorkshop(id, data) {
  ...
  const result = await sql`
    ...
    SET
      ...
      price_couple = ${data.priceCouple || null},
