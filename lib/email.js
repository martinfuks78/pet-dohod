...
export async function sendRegistrationConfirmation(registration, workshop) {
  try {
    ...
    // Vygenerovat iCal attachment
    const icsBase64 = getICSBase64({
      workshopDate: registration.workshopDate,
      workshopLocation: registration.workshopLocation,
    ...
    const info = await transporter.sendMail({
      ...
      attachments: [
        {
          filename: 'workshop-pet-dohod.ics',
...
export async function sendAdminNotification(registration) {
  try {
    const info = await transporter.sendMail({
      from: EMAIL_FROM,
      to: process.env.ADMIN_EMAIL || 'kouc@martinfuks.cz',
 ...
 */
export async function sendWaitlistConfirmation(registration) {
  try {
    // Validace základních údajů
    if (!registration || !registration.email || !registration.firstName) {
      throw new Error('Missing required registration data')
